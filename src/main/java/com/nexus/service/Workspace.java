...
public class Workspace {
    ...
    public static void addProjectToMap(String projectName, Project project) {
        if (getProjectFromName(projectName) != null) return;
        projectFromNameMap.put(projectName,project);
    }
    ...
    List<User> list = taskCountByUser.entrySet().stream()
    ...
    return taskCountByStatus.entrySet().stream()
        ...
        .limit(1)
        .filter(entry -> entry.getKey() != TaskStatus.DONE)
